...
| **变换** | 随机打乱点击处 3×3 九格内方块的排列，把散开的同色块凑到一起 |
...
- **第 6 关和第 11 关**有**游戏任务**：指定几种颜色各消除 6 个，每完成一项奖励 1000 分（第 11 关之后每 5 关再出现一次）
...
| 模式 | 说明 |
|---|---|
| **原版还原** | 1318×1079 的固定画面，底板取自一张无水印的原版截图，棋盘精确落在原坐标 (10,231)、格距 77.4px，与原版逐像素对位。横屏或桌面自动启用 |
...
- **`tools/rip-clean.py`** 从一张无水印的原版截图里抠高清方块（77px，比录屏清晰近一倍）。虽然只有一帧，但同一种方块在棋盘上有二十来个实例，对它们中位数叠加同样能压掉 JPEG 噪点。
...
`assets/blocks/` 下的方块贴图提取自原版游戏录屏，`assets/stage/` 下的界面底板取自一张原版截图（做法见 `tools/rip.py`、`tools/make-stage.py`），著作权属于联众。这是一个学习与怀旧性质的复刻，**请勿用于商业用途**。
